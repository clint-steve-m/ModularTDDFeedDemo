...
class LoadFeedFromRemoteUseCaseTests: XCTestCase {
    ...
    func test_loadTwice_requestsDataFromURLTwice() {
        ...
        // comparing two arrays for equality - order, equality and count are tested at the same time
    ...
    private func makeItem(id: UUID, description: String? = nil, location: String? = nil, imageURL: URL) ->  (model: FeedImage, json: [String: Any]) {
        ...
        let json = [
            ...
            ].reduce(into: [String:Any]()) { (accumaledDict, element) in
                if let value = element.value { accumaledDict[element.key] = value }
        }
        return (item, json)
    }
    ...
    private class HTTPClientSpy: HTTPClient {
        ...
        func complete(withStatusCode code: Int, data: Data, at index: Int = 0) {
            let response = HTTPURLResponse(
                ...
                httpVersion: nil,
                headerFields: nil
                )! // hardcoded HTTPURLResponse is always valid
