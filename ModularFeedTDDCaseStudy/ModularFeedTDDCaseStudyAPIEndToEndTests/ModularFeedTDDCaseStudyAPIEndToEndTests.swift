...
class ModularFeedTDDCaseStudyAPIEndToEndTests: XCTestCase {
    
    func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
        switch getFeedResult() {
        case let .success(items)?:
            XCTAssertEqual(items.count, 8, "Expected 8 items in the test account feed")
            // since data is static, error can be pinpointed
            ...
            // in the case the data is not static, enumarate
...
//            }
    ...
    private func getFeedResult(file: StaticString = #file, line: UInt = #line) -> LoadFeedResult? {
        let testServerURL = URL(string: "https://api.myjson.com/bins/c9nia")!
